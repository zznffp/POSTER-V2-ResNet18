...
try:
    ...
    from vit_model_caers import VisionTransformer, PatchEmbed
except ImportError:
    from .mobilefacenet import MobileFaceNet
    from .vit_model_caers import VisionTransformer, PatchEmbed
...
class PosterV2_ResNet(nn.Module):
    def __init__(self, img_size=224, num_classes=7, dims=[64, 128, 256], window_size=[28, 14, 7], dropout=0.0, use_csi=True):
        super().__init__()
    ...
    def forward(self, x, return_features=False, return_all_layers=False, return_attention=False, return_adapted=False):
        # Face Branch
        ...
        # Save adapted features for NA-MSAC (merged forward)
        x1_adapted, x2_adapted, x3_adapted = x1, x2, x3
        ...
        if return_attention:
            ...
            if return_adapted:
                return output, aux_output, [attn_map1, attn_map2, attn_map3], \
                       (x1_adapted, x2_adapted, x3_adapted)
            return output, aux_output, [attn_map1, attn_map2, attn_map3]
