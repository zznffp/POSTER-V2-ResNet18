...
from .vit_model_9 import VisionTransformer, PatchEmbed
...
class pyramid_trans_expr2(nn.Module):
    def __init__(self, img_size=224, num_classes=9, window_size=[28, 14, 7], num_heads=[2, 4, 8], dims=[64, 128, 256],
                 embed_dim=768):
        super().__init__()
    ...
    def forward(self, x, return_features=False):
        ...
        if return_features:
            out = self.VIT(o, return_features=True)  # return features for distillation
        else:
            out = self.VIT(o)
        return out
