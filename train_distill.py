...
try:
    ...
    MIXUP_AC_AVAILABLE = True
except ImportError:
    print("Warning: Mixup AC module not found. --use_mixup_ac will be disabled.")
...
def main():
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_acc, train_loss = train_distill(train_loader, student, teacher, criterion_focal,
                                              ...
                                              mixup_ac_module=mixup_ac_module, na_msac_module=na_msac_module,
...
def train_distill(train_loader, student, teacher, criterion_hard, criterion_contrastive, optimizer, epoch, scaler, args,
                  log_file=None, feature_projector=None, mixup_ac_module=None, na_msac_module=None,
    ...
    mixup_ac_losses = AverageMeter('MixupAC', ':.4f')
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        with torch.cuda.amp.autocast():
            ...
            if mixup_ac_module is not None:
                x1, x2, x3 = student.forward_backbone(images)
            ...
            if use_mixup_this_batch and mixup_ac_module is not None:
                x1_mixed, x2_mixed, x3_mixed = x1, x2, x3

                x1_a, x2_a, x3_a = student.forward_backbone(images_original)

                x1_b, x2_b, x3_b = student.forward_backbone(images_b)
            ...
            if use_mixup_this_batch and mixup_ac_module is not None:
                mixup_ac_loss, mixup_ac_loss_dict = mixup_ac_module(
                    x1_mixed, x2_mixed, x3_mixed,
                    x1_a, x2_a, x3_a,
                    x1_b, x2_b, x3_b,
                    lam
                )
            else:
                mixup_ac_loss = torch.tensor(0.0).cuda()
                mixup_ac_loss_dict = {'mixup_ac_loss_total': 0.0}

            total_loss = dist_loss + args.beta * aux_loss + args.lambda_proto * proto_loss + args.lambda_mixup_ac * mixup_ac_loss + args.lambda_na_msac * na_msac_loss
        ...
        mixup_ac_losses.update(mixup_ac_loss_dict['mixup_ac_loss_total'], images.size(0))
        ...
        if i % 50 == 0:
            if na_msac_module is not None:
                progress_msg = (f"  Epoch [{epoch}][{i}/{len(train_loader)}] "
                                ...
                                f"MixupAC: {mixup_ac_losses.avg:.4f}) | "
                                f"Acc: {top1.avg:.2f}%")
            elif mixup_ac_module is not None:
                progress_msg = (f"  Epoch [{epoch}][{i}/{len(train_loader)}] "
                                f"Loss: {losses.avg:.4f} (Hard: {hard_losses.avg:.4f}, Soft: {soft_losses.avg:.4f}, "
                                f"Feat: {feature_losses.avg:.4f}, Contrast: {contrastive_losses.avg:.4f}, "
